...
class RemoteConnector(BaseConnector):
    ...
    async def scan(self, options: Dict[str, Any]) -> Dict[str, Any]:
        try:
            ...
            logger.error("Remote scan failed: %s", str(e))
            raise
    ...
    async def analyze(self, options: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error("Remote analyze failed: %s", str(e))
            raise
        ...
        try:
            async with httpx.AsyncClient() as client:
                response = await client.post(
                    ...
                    json=payload,
                    headers=self.headers,
                    timeout=300.0 # 5 minutes timeout for run
                )
                response.raise_for_status()
            ...
            logger.error("Remote run failed: %s", str(e))
            raise
